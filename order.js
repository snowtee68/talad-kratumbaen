(() => {
  ...
  function injectUI(){
    if(!document.querySelector('link[href*="order.css"]')){const css=document.createElement('link');css.rel='stylesheet';css.href='order.css?v=0.3.0';document.head.appendChild(css);}
  ...
  async function addProduct(productId){
    ...
    const optionsHtml=gs.map(g=>{const min=Math.max(Number(g.min_select||0),g.required?1:0),max=g.selection_type==='single'?1:Math.max(1,Number(g.max_select||1)),type=g.selection_type==='single'?'radio':'checkbox',name=`og_${g.id}`;return `<fieldset class="payment-card option-group" data-option-group="${g.id}" data-min="${min}" data-max="${max}"><legend><b>${esc(g.name)}</b> ${min>0?'<span class="status-pill">ต้องเลือก</span>':'<span class="mo-muted">ไม่บังคับ</span>'}</legend><div class="mo-muted">${g.selection_type==='single'?'เลือกได้ 1 รายการ':`เลือกได้สูงสุด ${max} รายการ`}</div>${g.values.map(v=>`<label style="display:flex;gap:8px;align-items:center;padding:7px 0"><input data-option-value type="${type}" name="${name}" value="${v.id}" data-group-id="${g.id}" data-group-name="${esc(g.name)}" data-value-name="${esc(v.name)}" data-price-delta="${Number(v.price_delta||0)}"> <span>${esc(v.name)}${Number(v.price_delta||0)?` <b>+${money(v.price_delta)} บาท</b>`:''}</span></label>`).join('')||'<div class="warning-banner">กลุ่มนี้ยังไม่มีตัวเลือก กรุณาแจ้งร้าน</div>'}</fieldset>`}).join('');
    openModal(`<input id="customProductId" type="hidden" value="${p.id}"><input id="customShopId" type="hidden" value="${p.shop_id}"><input id="customProductName" type="hidden" value="${esc(p.name)}"><input id="customShopName" type="hidden" value="${esc(p.shop?.name||'ร้านค้า')}"><input id="customBasePrice" type="hidden" value="${Number(p.price)}"><input id="customImageUrl" type="hidden" value="${esc(p.image_url||'')}"><h2 class="mo-title">${esc(p.name)}</h2><div class="price" style="font-size:22px;font-weight:800;margin-bottom:10px">เริ่มต้น ${money(p.price)} บาท</div>${optionsHtml}<label style="display:block;margin-top:12px"><b>หมายเหตุถึงร้าน</b> <span class="mo-muted">(ไม่บังคับ สูงสุด 200 ตัวอักษร)</span><textarea id="productLineNote" maxlength="200" rows="3" placeholder="เช่น ไม่ใส่ผัก / แยกน้ำ / หวานน้อยเป็นพิเศษ"></textarea></label><div class="mo-actions"><button id="confirmAddConfiguredProduct" class="mo-primary">+ ใส่ตะกร้า</button></div>`,true);
  }
  function confirmAddConfiguredProduct(){
    ...
    if(found)found.qty+=1;else c.push({line_id:uuid(),signature,product_id,shop_id,shop_name,name,base_price,option_total,price,image_url,selected_options:selected,note,qty:1});saveCart(c);closeModal();alert('เพิ่มลงตะกร้าแล้ว');
  }
  ...
  async function saveProduct(){
    ...
    const payload={shop_id:shopId,name,price,description:description||null,active,sale_status,sort_order:sort,image_url:image,image_path:imagePath,updated_at:new Date().toISOString()};const q=id?db.from('market_products').update(payload).eq('id',id):db.from('market_products').insert(payload);const {error}=await q;if(error){if(newImagePath)await safeRemove('shop-images',newImagePath);return alert(error.message)}if(newImagePath&&oldImagePath&&oldImagePath!==newImagePath)await safeRemove('shop-images',oldImagePath);alert('บันทึกสินค้าแล้ว');await refreshProductShops();openSellerShop(shopId);
  }
