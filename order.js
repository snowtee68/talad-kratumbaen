(() => {
  ...
  const ORDER_UI_VERSION='0.5.20.10';
  ...
  function decorateShopCards(){
    ...
    document.querySelectorAll('.card[data-id]').forEach(card=>{
      ...
      const b=document.createElement('button');b.className='market-order-btn';b.dataset.marketOrderShop=id;b.textContent='🛒 สั่งซื้อผ่านตลาด';b.style.cssText='display:block;width:calc(100% - 20px);max-width:560px;margin:12px auto 4px;border:0;border-radius:12px;background:#8f0d12;color:#fff;padding:14px 18px;min-height:52px;text-align:center;font:800 17px Prompt,sans-serif;box-shadow:0 5px 14px rgba(143,13,18,.28);cursor:pointer';actions.appendChild(b);
    });
    const detail=document.getElementById('detailSummary');
    if(detail&&!detail.querySelector('[data-market-order-shop]')){
      const id=document.getElementById('reviewShopId')?.value;if(id&&productShopIds.has(String(id))){const b=document.createElement('button');b.className='market-order-btn';b.dataset.marketOrderShop=id;b.textContent='🛒 ดูสินค้าและสั่งซื้อ';b.style.cssText='display:block;width:calc(100% - 20px);max-width:560px;margin:10px auto 14px;border:0;border-radius:12px;background:#8f0d12;color:#fff;padding:14px 18px;min-height:52px;text-align:center;font:800 17px Prompt,sans-serif;box-shadow:0 5px 14px rgba(143,13,18,.28);cursor:pointer';detail.prepend(b);}
    }
  }

  async function openShopMenu(shopId){
    if(!canUseOrders())return;
    const [{data:shop},{data:setting},{data:access},{data:products,error}]=await Promise.all([
  ...
  async function getOrderPushRegistration(){
    ...
    return navigator.serviceWorker.register('./sw.js?v=5.7.9.25',{scope:'./',updateViaCache:'none'});
  }
