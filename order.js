(() => {
  ...
  async function renderCustomerHub(box){
    ...
    box.innerHTML=(groups||[]).map(g=>{const os=g.orders||[],activeOrders=os.filter(o=>o.status!=='cancelled'),readyCount=activeOrders.filter(o=>o.status==='ready').length,waitingCount=Math.max(0,activeOrders.length-readyCount),allReady=activeOrders.length>0&&waitingCount===0,groupOpen=!['cancelled','completed'].includes(g.status),canDelivery=allReady&&groupOpen&&!g.rider_job_id;const deliveryState=g.rider_job_id?`<div class="delivery-cost-box">🛵 สร้างงานวินแล้ว #${esc(String(g.rider_job_id).slice(0,8).toUpperCase())}</div>`:canDelivery?`<div class="ready-banner">✅ ทุกร้านพร้อมแล้ว สามารถเรียกวินให้ไปรับรวมเที่ยวเดียวได้</div><button class="mo-primary" data-create-delivery="${g.id}">🛵 เรียกวินรับของทุกร้าน</button>`:groupOpen&&activeOrders.length?`<div class="warning-banner">⏳ รออีก ${waitingCount} ร้านพร้อมรับสินค้า (${readyCount}/${activeOrders.length} ร้านพร้อม)</div><button class="mo-primary" disabled aria-disabled="true" title="ต้องรอทุกร้านพร้อมก่อนเรียกวิน" style="opacity:.5;cursor:not-allowed">🛵 รอทุกร้านพร้อมก่อนเรียกวิน</button>`:'';return `<article class="order-card"><div class="order-card-head"><div><b>ชุดคำสั่งซื้อ #${esc(String(g.id).slice(0,8).toUpperCase())}</b><div class="mo-muted">${new Date(g.created_at).toLocaleString('th-TH')}</div></div><span class="status-pill">${esc(g.status||'กำลังดำเนินการ')}</span></div>${os.map(o=>`<div class="payment-card"><div class="order-card-head"><b>🏪 ${esc(o.shop?.name||'ร้าน')}</b><span class="status-pill status-${esc(o.status)}">${esc(statusText(o.status))}</span></div><div class="order-items">${(o.items||[]).map(i=>`${esc(i.product_name)} × ${i.qty} = ${money(Number(i.unit_price)*i.qty)} บาท`).join('<br>')}</div><b>${money(o.subtotal)} บาท</b>${o.status==='awaiting_payment'?`<div class="mo-actions"><button class="mo-primary" data-pay-order="${o.id}">ชำระ/แจ้งชำระเงิน</button></div>`:''}</div>`).join('')}${deliveryState}</article>`}).join('')||'<p>ยังไม่มีออเดอร์</p>';
  }
