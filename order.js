(() => {
  ...
  async function submitCheckout(){
    ...
    const {data,error}=await db.rpc('market_create_checkout_v040',{p_customer_name:name,p_customer_phone:phone,p_fulfillment_method:method,p_delivery_address:method==='delivery'?address:null,p_delivery_lat:method==='delivery'?lat:null,p_delivery_lng:method==='delivery'?lng:null,p_pickup_requested_at:pickupAt,p_orders:payload});
  ...
  async function renderCustomerHub(box){
    ...
    box.innerHTML=(groups||[]).map(g=>{
      ...
      return `<article class="order-card"><div class="order-card-head"><div><b>ชุดคำสั่งซื้อ #${esc(String(g.id).slice(0,8).toUpperCase())}</b><div class="mo-muted">${new Date(g.created_at).toLocaleString('th-TH')}</div></div><span class="status-pill">${esc(g.status||'กำลังดำเนินการ')}</span></div>${os.map(o=>{
        let refund='';
        if(o.refund_required){if((o.refund_status||'pending')==='pending')refund=`<div class="warning-banner">🔄 ร้านต้องคืนเงิน ${money(o.refund_amount||o.subtotal)} บาท กรุณารอร้านดำเนินการ</div>`;else if(o.refund_status==='seller_submitted')refund=`<div class="ready-banner">💸 ร้านแจ้งคืนเงิน ${money(o.refund_amount||o.subtotal)} บาทแล้ว</div><div class="mo-actions">${o.refund_slip_path?`<button class="mo-secondary" onclick="window.marketOrderOpenRefundSlip('${o.id}','${esc(o.refund_slip_path)}')">ดูหลักฐานคืนเงิน</button>`:''}<button class="mo-primary" data-confirm-refund="${o.id}">✅ ได้รับเงินคืนแล้ว</button></div>`;else if(o.refund_status==='completed')refund=`<div class="ready-banner">✅ ยืนยันได้รับเงินคืน ${money(o.refund_amount||o.subtotal)} บาทแล้ว</div>`;}
