(() => {
  ...
  async function openCheckout(){
    ...
    const by=Object.fromEntries((settings||[]).map(s=>[s.shop_id,s]));const unavailable=groups.map(g=>({g,av:shopAvailability(by[g.shop_id],shopBy?.[g.shop_id]||null)})).filter(x=>!x.av.ok);
  ...
  async function submitCheckout(){
    ...
    const groups=groupedCart();const selectedCoupons=selectedCheckoutCoupons();const payload=groups.map(g=>({shop_id:g.shop_id,items:g.items.map(x=>({product_id:x.product_id,qty:x.qty,option_value_ids:(x.selected_options||[]).map(o=>o.value_id),note:x.note||null}))}));
    if(method==='delivery'){
    ...
    if(selectedCoupons.length){
      const {data:couponResult,error:couponError}=await db.rpc('market_apply_mission_coupons_to_group',{p_group_id:createdGroupId,p_coupon_ids:selectedCoupons});
      if(couponError){console.error('coupon apply',couponError);alert('ออเดอร์ถูกสร้างแล้ว แต่ใช้คูปองไม่สำเร็จ: '+couponError.message+'\nกรุณาตรวจสอบยอดก่อนชำระเงิน');}
      else if(couponResult?.discount_total>0){console.info('coupon discount',couponResult.discount_total);}
    }
  ...
  async function getOrderPushRegistration(){
    ...
    return navigator.serviceWorker.register('./sw.js?v=0.5.22.1',{scope:'./',updateViaCache:'none'});
  }
