(() => {
  ...
  const ORDER_UI_VERSION='0.5.8.1';
  ...
  async function showCheckoutResult(groupId){
    const {data:orders,error}=await db.from('market_orders').select('id,shop_id,subtotal,status,shop:market_shops(name,phone)').eq('group_id',groupId).order('created_at');
    if(error)return alert(error.message);
    openModal(`<h2 class="mo-title">ส่งคำสั่งซื้อแล้ว ✅</h2><div class="ready-banner">ตอนนี้ยังไม่ต้องโอนเงิน ระบบจะรอให้แต่ละร้านตรวจและกดรับออเดอร์ก่อน</div>${(orders||[]).map(o=>`<div class="payment-card"><b>🏪 ${esc(o.shop?.name||'ร้านค้า')}</b><div class="payment-amount">${money(o.subtotal)} บาท</div><div class="warning-banner">⏳ รอร้านรับออเดอร์</div>${o.shop?.phone?`<a class="mo-secondary" style="display:inline-block;text-decoration:none" href="tel:${esc(o.shop.phone)}">📞 โทรหาร้าน</a>`:''}</div>`).join('')}<div class="mo-actions"><button class="mo-secondary" data-hub-tab="customer">ดูสถานะออเดอร์</button></div>`,true);
  }
  ...
  function customerGroupCard(g){
    ...
    const deliveryState=pickup?(allReady?`<div class="ready-banner">🏪 สินค้าพร้อมรับครบ ${readyCount}/${activeOrders.length} ร้านแล้ว${g.pickup_requested_at?`<br><small>เวลาที่ขอรับ: ${new Date(g.pickup_requested_at).toLocaleString('th-TH')}</small>`:''}<br><small>กรุณาไปรับสินค้าตามร้านในรายการด้านบน</small></div>`:`<div class="warning-banner">🏪 รับเองที่ร้าน · พร้อม ${readyCount}/${activeOrders.length} ร้าน${g.pickup_requested_at?`<br><small>เวลาที่ขอรับ: ${new Date(g.pickup_requested_at).toLocaleString('th-TH')}</small>`:''}</div>`):g.rider_job_id?`<div class="delivery-cost-box">🛵 สร้างงานวินแล้ว #${esc(String(g.rider_job_id).slice(0,8).toUpperCase())}</div>`:canDelivery?(activeOrders.length>1?`<div class="ready-banner">✅ พร้อมครบ ${readyCount}/${activeOrders.length} ร้าน สามารถเรียกวินไปรับรวมเที่ยวเดียวได้</div><button class="mo-primary" data-create-delivery="${g.id}">🛵 เรียกวินรับของทั้ง ${activeOrders.length} ร้าน</button>`:`<div class="ready-banner">✅ ร้านพร้อมรับสินค้าแล้ว สามารถเรียกวินได้</div><button class="mo-primary" data-create-delivery="${g.id}">🛵 เรียกวินรับสินค้า</button>`):groupOpen&&activeOrders.length?`<div class="warning-banner">⏳ รออีก ${waitingCount} ร้านพร้อมรับสินค้า (${readyCount}/${activeOrders.length} ร้านพร้อม)</div><button class="mo-primary" disabled style="opacity:.5">🛵 รอทุกร้านพร้อมก่อนเรียกวิน</button>`:'';
    return `<article class="order-card"><div class="order-card-head"><div><b>ชุดคำสั่งซื้อ #${esc(String(g.id).slice(0,8).toUpperCase())}</b><div class="mo-muted">${new Date(g.created_at).toLocaleString('th-TH')}</div></div><span class="status-pill">${esc(g.status||'กำลังดำเนินการ')}</span></div>${os.map(o=>{
      let refund='';
      ...
      const pending=o.status==='pending_shop'?`<div class="warning-banner">${overdue?'⚠️ ร้านยังไม่ตอบรับเกิน 15 นาที':'⏳ รอร้านตรวจและรับออเดอร์'}${o.shop?.phone?`<br><a href="tel:${esc(o.shop.phone)}">📞 โทรหาร้าน</a>`:''}</div><div class="mo-actions"><button class="mo-danger" data-cancel-shop-order="${o.id}">ยกเลิกร้านนี้</button></div>`:'';
  ...
  function sellerOrderBucket(o){
    if(o.status==='cancelled'||o.group?.status==='completed'||o.refund_status==='completed')return 'done';
  ...
  async function createDelivery(groupId){
    if(!session)return requireLogin();
    const {data:g,error}=await db.from('market_delivery_groups').select('*,orders:market_orders(id,status,shop:market_shops(id,name,latitude,longitude,phone,landmark,address))').eq('id',groupId).eq('customer_id',session.user.id).maybeSingle();
    if(error||!g)return alert('ไม่พบชุดคำสั่งซื้อ');
    if(g.fulfillment_method==='pickup')return alert('ออเดอร์นี้เลือกรับเองที่ร้าน จึงไม่ต้องเรียกวิน');
    const orders=(g.orders||[]).filter(o=>o.status!=='cancelled'),notReady=orders.filter(o=>o.status!=='ready');
    if(!orders.length)return alert('ไม่มีออเดอร์ที่สามารถเรียกวินได้');
    if(notReady.length)return alert(`ยังเรียกวินไม่ได้ ต้องรออีก ${notReady.length} ร้านพร้อมรับสินค้าก่อน`);
    if(g.rider_job_id)return alert('สร้างงานวินแล้ว');
    ...
    const stops=[...pickups,drop],km=routeKm(stops),fare=fareFor(km,pickups.length);if(!fare)return alert('เส้นทางรวมเกิน 10 กม. ซึ่งเกินพื้นที่ทดสอบของระบบวิน');
    if(!confirm(`เรียกวินไปรับ ${pickups.length} ร้าน แล้วส่งเที่ยวเดียว\nระยะทางประมาณ ${km.toFixed(1)} กม.\nค่าจัดส่งประมาณ ${fare.total} บาท\n\nค่าส่งชำระให้วินเมื่อได้รับสินค้า`))return;
    const {data:claimed,error:claimErr}=await db.rpc('market_claim_delivery_creation',{p_group_id:groupId});
    if(claimErr)return alert('ยังเรียกวินไม่ได้: '+claimErr.message);
    if(!claimed)return alert('มีการสร้างงานวินสำหรับชุดนี้อยู่แล้ว กรุณารอสักครู่แล้วรีเฟรช');
    const release=async()=>{try{await db.rpc('market_release_delivery_creation',{p_group_id:groupId})}catch(_){}};
    const {data:jobId,error:jobErr}=await db.rpc('rider_create_multistop_job',{p_stops:stops,p_job_note:`ชุดคำสั่งซื้อ ${String(groupId).slice(0,8).toUpperCase()}`,p_payer:'receiver',p_distance_km:+km.toFixed(3),p_fare_estimate:fare.total,p_extra_stop_fee:fare.extra});
    if(jobErr){await release();return alert('สร้างงานวินไม่สำเร็จ: '+jobErr.message)}
    const {error:up}=await db.rpc('market_attach_rider_job',{p_group_id:groupId,p_rider_job_id:jobId,p_delivery_fee:fare.total,p_distance_km:+km.toFixed(3)});
    if(up){await release();return alert('สร้างงานวินแล้ว แต่ผูกกับออเดอร์ไม่สำเร็จ: '+up.message)}
    alert(`เรียกวินแล้ว ค่าส่งประมาณ ${fare.total} บาท`);openAccountHub('customer');
  }
