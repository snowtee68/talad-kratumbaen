(() => {
  ...
  function wire(){
    ...
    document.addEventListener('click',e=>{
      ...
      const cancelProblem=e.target.closest('[data-cancel-problem-shop]');if(cancelProblem)return customerCancelProblemShop(cancelProblem.dataset.cancelProblemShop);const ship=e.target.closest('[data-create-delivery]');if(ship)return createDelivery(ship.dataset.createDelivery);
  ...
  function deliveryBatchCard(b){
    const phone=b.rider_phone||'',done=b.status==='completed';
    return `<div class="delivery-track"><div class="order-card-head"><b>${done?'✅':'🛵'} ${esc(deliveryBatchStatusText(b.status))}</b><span class="status-pill">${esc(String(b.id).slice(0,8).toUpperCase())}</span></div>${b.rider_name||phone?`<div class="rider-contact"><b>วิน: ${esc(b.rider_name||'ไม่ระบุชื่อ')}</b>${phone?` · ${esc(phone)} <a href="tel:${esc(phone)}">📞 โทรหาวิน</a>`:''}</div>`:`<div class="mo-muted">รอวินรับงานและส่งข้อมูลติดต่อ</div>`}<div class="delivery-steps"><span class="${['accepted','pickup_started','picked_up','delivering','completed'].includes(b.status)?'done':''}">วินรับงาน</span><span class="${['picked_up','delivering','completed'].includes(b.status)?'done':b.status==='pickup_started'?'active':''}">รับสินค้า</span><span class="${b.status==='delivering'?'active':['completed'].includes(b.status)?'done':''}">กำลังส่ง</span><span class="${b.status==='completed'?'done':''}">ส่งสำเร็จ</span></div>${b.delivery_fee?`<small>ค่าส่งประมาณ ${money(b.delivery_fee)} บาท${b.distance_km?` · ${Number(b.distance_km).toFixed(1)} กม.`:''}</small>`:''}</div>`;
  }
  ...
  function customerGroupCard(g){
    ...
    const deliveryState=pickup?(allReady?`<div class="ready-banner">🏪 สินค้าพร้อมรับครบ ${readyCount}/${activeOrders.length} ร้านแล้ว${g.pickup_requested_at?`<br><small>เวลาที่ขอรับ: ${new Date(g.pickup_requested_at).toLocaleString('th-TH')}</small>`:''}</div>`:`<div class="warning-banner">🏪 รับเองที่ร้าน · พร้อม ${readyCount}/${activeOrders.length} ร้าน</div>`):readyUnbatched.length?`<div class="mo-actions"><button class="mo-primary" data-create-delivery="${g.id}">🛵 ${notReadyUnbatched.length?`ส่งเฉพาะ ${readyUnbatched.length} ร้านที่พร้อม`:`เรียกวินรับสินค้า ${readyUnbatched.length} ร้าน`}</button></div>`:unbatched.length?`<button class="mo-primary" disabled style="opacity:.5">🛵 รอร้านพร้อมก่อนเรียกวิน</button>`:'';
  ...
  function sellerOrderDeliveryInfo(o){
    ...
    const phone=batch.rider_phone||'';
    return `<div class="delivery-track"><b>🛵 ${esc(deliveryBatchStatusText(batch.status))}</b>${batch.rider_name||phone?`<div class="rider-contact"><b>วิน: ${esc(batch.rider_name||'ไม่ระบุชื่อ')}</b>${phone?` · ${esc(phone)} <a href="tel:${esc(phone)}">📞 โทรหาวิน</a>`:''}</div>`:`<div class="mo-muted">รอวินรับงาน</div>`}</div>`;
  }
