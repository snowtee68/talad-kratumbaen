(() => {
  ...
  let session=null, productShopIds=new Set(), productOptionDraft=[], productSaveInProgress=false;
...
if(e.target.closest('#showDeliveryFareInfoBtn'))return showDeliveryFareInfo(false);if(e.target.closest('#closeDeliveryFareInfoBtn'))return closeModal();
    ...
    document.addEventListener('click',e=>{
      ...
      const copyp=e.target.closest('[data-copy-product]');if(copyp)return openProductEditor(copyp.dataset.copyProduct,true);
  ...
  async function openShopMenu(shopId){
    ...
    const defaultCategory=visibleCats.length?String(visibleCats[0].id):'all';
  ...
  async function openProductEditor(productId=null,copyMode=false){
    const guardShopId=document.getElementById('sellerShopId')?.value;
    if(guardShopId&&!await ensureSellerDeliveryAccess(guardShopId))return;
    const shopId=document.getElementById('sellerShopId')?.value;if(!shopId)return;
    let p={};
    if(productId){const {data,error}=await db.from('market_products').select('*').eq('id',productId).maybeSingle();if(error)return alert(error.message);p=data||{};}
    const {data:categories,error:catErr}=await db.from('market_product_categories').select('*').eq('shop_id',shopId).eq('active',true).order('sort_order').order('created_at');if(catErr)return alert(catErr.message);
    try{await loadProductOptionDraft(productId)}catch(err){return alert('โหลดตัวเลือกสินค้าไม่สำเร็จ: '+err.message)}
    const st=p.sale_status||(p.active===false?'sold_out':'available');
    const editorId=copyMode?'':(p.id||'');
    const editorName=copyMode?(p.name?`${p.name} (สำเนา)`:''):(p.name||'');
    const title=copyMode?'ทำสำเนาสินค้า':(productId?'แก้ไขสินค้า':'เพิ่มสินค้า');
    const copyNotice=copyMode?'<div class="ready-banner" style="margin-bottom:12px">📄 คัดลอกราคา หมวด รายละเอียด สถานะ และตัวเลือกย่อยมาให้แล้ว<br><b>กรุณาเปลี่ยนชื่อและเลือกรูปสินค้าใหม่</b> · ระบบจะไม่คัดลอกไฟล์รูปเดิม เพื่อไม่ให้กระทบสินค้าต้นฉบับ</div>':'';
    productSaveInProgress=false;
    openModal(`<input id="productShopId" type="hidden" value="${esc(shopId)}"><input id="productId" type="hidden" value="${esc(editorId)}"><h2 class="mo-title">${title}</h2>${copyNotice}<div class="mo-form two product-editor-grid"><label class="product-name-field">ชื่อสินค้า *<input id="productName" value="${esc(editorName)}"></label><label>ราคา (บาท) *<input id="productPrice" type="number" min="0" step="0.01" value="${esc(p.price??'')}"></label><label>หมวดหมู่<select id="productCategory"><option value="">อื่น ๆ / ยังไม่กำหนด</option>${(categories||[]).map(c=>`<option value="${esc(c.id)}" ${String(p.category_id||'')===String(c.id)?'selected':''}>${esc(c.name)}</option>`).join('')}</select></label><label>สถานะสินค้า<select id="productSaleStatus"><option value="available" ${st==='available'?'selected':''}>🟢 เปิดขาย</option><option value="sold_out" ${st==='sold_out'?'selected':''}>🟠 หมดชั่วคราว</option><option value="discontinued" ${st==='discontinued'?'selected':''}>⚫ เลิกขาย / หมดถาวร</option></select></label><label>ลำดับ<input id="productSort" type="number" value="${Number(p.sort_order||0)}"></label><label class="full">รายละเอียด<textarea id="productDescription" rows="3">${esc(p.description||'')}</textarea></label><label class="full">รูปสินค้า (1 รูป / ระบบย่อเป็น WebP อัตโนมัติ)<input id="productImage" type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif"><small class="mo-muted">${copyMode?'ทำสำเนา: กรุณาเลือกรูปใหม่หากต้องการให้สินค้ามีรูป · ':''}“หมดชั่วคราว” ยังเห็นรายการในหลังร้านและเปิดกลับได้ทันที ส่วน “เลิกขาย” จะซ่อนจากลูกค้าแต่ไม่ลบประวัติ</small></label></div><section class="seller-section" style="margin-top:14px"><div class="order-card-head"><div><h3 style="margin:0">⚙️ ตัวเลือกสินค้า</h3><div class="mo-muted">ไม่บังคับ · เช่น ความหวาน ความเผ็ด เพิ่มไข่ หรือ Topping</div></div><button type="button" id="draftAddOptionGroup" class="mo-primary">+ เพิ่มกลุ่มตัวเลือก</button></div><div id="productOptionsDraft"></div></section><div class="mo-actions"><button id="saveProductBtn" class="mo-primary">${copyMode?'สร้างสินค้าจากสำเนา':'บันทึกสินค้าและตัวเลือก'}</button></div>`);
    renderProductOptionDraft();
  }
  ...
  async function saveProduct(){
    ...
    const originalBtnText=saveBtn?.textContent||'บันทึกสินค้าและตัวเลือก';
    if(saveBtn){saveBtn.disabled=true;saveBtn.textContent='กำลังบันทึก...';saveBtn.setAttribute('aria-busy','true');}
    let newImagePath=null,oldImagePath=null;
    try{
      const shopId=document.getElementById('productShopId')?.value,id=document.getElementById('productId')?.value||'',name=document.getElementById('productName')?.value.trim()||'',price=Number(document.getElementById('productPrice')?.value),description=document.getElementById('productDescription')?.value.trim()||'',category_id=document.getElementById('productCategory')?.value||null,sale_status=document.getElementById('productSaleStatus')?.value||'available',active=sale_status==='available',sort=Number(document.getElementById('productSort')?.value||0),file=document.getElementById('productImage')?.files?.[0];
      if(!shopId)return alert('ไม่พบข้อมูลร้าน กรุณาปิดแล้วเปิดหน้าสินค้าใหม่');
      if(!name||!Number.isFinite(price)||price<0)return alert('กรอกชื่อและราคาให้ถูกต้อง');
      if(!id){const {count,error:countErr}=await db.from('market_products').select('id',{count:'exact',head:true}).eq('shop_id',shopId);if(countErr)return alert(countErr.message);if(Number(count||0)>=100)return alert('ร้านนี้มีสินค้าครบ 100 รายการแล้ว ไม่สามารถเพิ่มสินค้าใหม่ได้');}
      const optionErr=validateProductOptionDraft();if(optionErr)return alert(optionErr);
      ...
      if(btn){btn.disabled=false;btn.textContent=originalBtnText;btn.removeAttribute('aria-busy');}
    }
  }
